...
switch ($action) {
    case 'create':
        ...
        // check empty inputs
        if (!empty($username) && !empty($firstname) && !empty($lastname) && !empty($password) && !empty($confirm_password) && isset($is_admin)) {
    ...
    case 'update':
        ...
        // check empty inputs
        if (!empty($username) && !empty($firstname) && !empty($lastname) && isset($is_admin)) {
            ...
            if ($count['username_count'] == 0) {
                ...
                $stmt = $pdo->prepare("UPDATE users SET username = ?, firstname = ?, lastname = ?, is_admin = ? WHERE id = ?");
                $updateUser = $stmt->execute([$username, $firstname, $lastname, $is_admin, $userId]);
